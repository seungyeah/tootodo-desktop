      ...
      <tbody class="relative text-center border">
         {#key $events}
            {#each $events as event, i}
               {@const event_start = getDuration(
                  $selectedDate.start,
                  parseDate(event.start_date),
               )}
               {@const event_duration = getDuration(
                  parseDate(event.start_date),
                  parseDate(event.end_date),
               )}
               {@const event_end = getDuration(
                  parseDate(event.end_date),
                  $selectedDate.end,
               )}
               {@const start_to_event_start = getDuration(
                  $selectedDate.end,
                  parseDate(event.start_date),
               )}
               {@const end_to_event_start = getDuration(
                  parseDate(event.end_date),
                  $selectedDate.start,
               )}

               {#if start_to_event_start > 0 || end_to_event_start > 0}<div
                  ></div>{:else}
                  <tr class="h-[30px] border-b">
                     {#if event_start == 0 && event_end == 0}
                        <td
                           colspan={event_duration + event_start + 1}
                           class:milestone={event.milestone}
                           class:event={!event.milestone}
                     ...
                     {:else}
                        <td colspan={event_start} class="border-b"></td>
                        <td
                           colspan={event_duration + 1}
                           class:milestone={event.milestone}
                           class:event={!event.milestone}
                           data-percent={event_duration + 1}
                           ><span class="text-xs opacity-80">#</span>{i + 1}</td
                        >
                        <td colspan={event_end} class="border-b"></td>
                     {/if}
