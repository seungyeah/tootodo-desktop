...
{#if note}
	<Dialog.Root open={false}>
			...
			>
				...
				<div class="flex space-x-1 -translate-x-1">
					{#each note.tags as tag}
						<Badge
							variant="outline"
							class=" h-[18px] shadow-sm translate-y-0.5 font-mono  px-1.5  text-xs font-medium border rounded-md border-b-neutral-950/30 border-r-neutral-950/30"
						>
							{tag.name}
		...
		>
			<Dialog.Header>
				<Dialog.Title
					class=" p-2 border-b-2 border-neutral-600 border-double flex flex-col space-y-2 "
				>
					<div
						class="relative flex w-full space-x-1 h-7 justify-center items-center"
					>
						<ItemInfo
							itemInfo={note.connectedItemInfo}
							class="absolute top-0 left-0"
						/>
						<div
							class="font-semibold text-neutral-800 w-full text-center"
						>
							{note.title}
						</div>
					</div>
					<div class="flex space-x-1 -translate-x-1">
						<Button
							class="h-[18px] shadow-sm text-xs bg-neutral-100 translate-y-0.5 font-mono p-0 pr-1 py-0 font-medium rounded-md hover:bg-neutral-100 "
							><Tag
								size={19}
								class="p-1 text-neutral-700"
							/><input
								class="w-9 h-3.5 p-1 rounded"
								placeholder="tag"
							/></Button
						>
						{#each note.tags as tag}
							<Badge
								variant="outline"
								class="h-[18px] shadow-sm text-xs translate-y-0.5 font-mono  px-1.5 py-0  font-medium border rounded-md border-b-neutral-950/30 border-r-neutral-950/30"
							>
								{tag.name}
								<span class="ml-1 flex">
									{#each tag.groups as group_id, i}
