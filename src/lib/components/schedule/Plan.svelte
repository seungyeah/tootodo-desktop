<script lang="ts">
   ...
   export let record = {
      ...
      category: { title: "Project K", color: "red" },
   };
</script>
...
<div class="flex-col">
   {#each tasks as task, i}
      <!-- schedule weekly -->
      <div class="flex h-10 my-2 border-l-2 border-r-2 rounded border-zinc-500">
         {#each weeks as day, i}
            <ContextMenu.Root
               ...
               onOpenChange={() => {
                  if (record.days.some((item) => item.day === day))
                     record.days.find((item) => item.day === day).from =
                        scheduledFrom;
                  record.days.find((item) => item.day === day).to = scheduledTo;
                  scheduledFrom = "";
                  scheduledTo = "";
               }}
            >
               <ContextMenu.Trigger class="w-full h-full -translate-y-1 "
                  ><button
                     ...
                     class="relative w-full h-full mt-1 border-l border-r border-dotted rounded shadow-sm border-zinc-300 hover:bg-zinc-100"
                     style={record.days.some((item) => item.day === day)
