<script lang="ts">
   ...
   // 오늘 날짜의 위치 계산
   let todayPosition = $derived(
      months.dates.findIndex(
         (date: CalendarDate) => date.compare(today(getLocalTimeZone())) === 0,
      ),
   );

   // 오늘 날짜의 상대적 위치 (퍼센트) 계산
   let todayPositionPercent = $derived(
...
<div
   class="w-full h-full max-h-full overflow-x-scroll overflow-y-scroll border-2 no-scrollbar border-neutral-800"
...
{#if todayPositionPercent}
   <div
      class=" pointer-events-none fixed z-10 h-[calc(100%-44px)] border-4 border-double top-[32px] bg-violet-100/10 border-neutral-400/10"
