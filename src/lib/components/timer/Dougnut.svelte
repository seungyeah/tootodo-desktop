    ...
    <svg viewBox="0 0 {size} {size}">
      {#each angles as { startAngle, endAngle }, i (i)}
        <path
          d="
            M {size / 2} {size / 2}
            L {size / 2 + Math.sin(startAngle) * (size / 2 - strokeWidth / 2)} {size / 2 - Math.cos(startAngle) * (size / 2 - strokeWidth / 2)}
            A {size / 2 - strokeWidth / 2} {size / 2 - strokeWidth / 2} 0 {endAngle - startAngle > Math.PI ? 1 : 0} 1 {size / 2 + Math.sin(endAngle) * (size / 2 - strokeWidth / 2)} {size / 2 - Math.cos(endAngle) * (size / 2 - strokeWidth / 2)}
            Z
          "
          fill={color(data[i], i)}
          stroke="none"
